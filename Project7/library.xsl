...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <section>
                ...
                <xsl:for-each select="//item[generate-id()=generate-id(key('collectionNames', collection)[1])]">
                    <xsl:sort select="collection" />
                    <h2>
    ...
    <xsl:value-of select="collection" />(<xsl:value-of select="count(key('collectionNames', collection))" />) |
    </xsl:template>
